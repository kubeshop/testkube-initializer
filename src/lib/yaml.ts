...
function buildEnterpriseValues(cfg: TestkubeConfig): Dict {
  ...
  // The AI service requires PostgreSQL; wire global.postgres when AI is on.
  if (aiEnabled) {
    global.postgres = buildAiPostgres(ai);
  }
  ...
  const values: Dict = {
    ...
    "testkube-ai-service": aiEnabled
      ...
      : { enabled: false },
    // Deploy the bundled PostgreSQL when AI uses the in-cluster backing store.
    ...
    ...(aiInCluster
      ? { postgresql: { enabled: true, auth: { database: "backend" } } }
      : {}),
    dex: {
